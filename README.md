...
├── prompts/                # プロンプト集
│   ├── coaching_L1.txt     #   L1: ソクラテス式コーチ
│   ├── coaching_L2.txt     #   L2: 沈黙のソクラテス
│   ├── coaching_L3.txt     #   L3: 鉄仮面のソクラテス
│   ├── teacher.txt         #   Teacher: 解説好きな先生
│   ├── retry_constraints.txt #  リトライ制約
│   ├── format_constraint.txt #  FORMAT_CONSTRAINT
...
Firebaseコンソール（https://console.firebase.google.com）の「プロジェクトの設定」→「サービスアカウント」から秘密鍵を生成し、プロジェクトルートに配置します。
...
アプリ起動後、サイドバーの入力欄にAPIキーを貼り付けてください。

Streamlit Cloudにデプロイする場合は、Settings → Secrets に以下を設定します。
...
```bash
# NewRAG（初回構築）: docs/ 配下の文書をベクトル化
python build_vector_db_for_NewRAG.py

# MixedRAG（差分更新・除霊）: 文書の追加・削除後に使用
...
### 1. Streamlit Cloud の Secrets にパスワードを追加
...
- [Vol.1: ソクラテスに説教されるシステムが爆誕した話](https://zenn.dev/mizunadad/articles/c6e53cfa8bc1a5)
- [Vol.2: Frontmatterで"学習指導要領"を定義する](https://zenn.dev/mizunadad/articles/d0b6c13fac6832)
- [Vol.3: 暴走するRAGソクラテスと、メタ認知の必要性](https://zenn.dev/mizunadad/articles/14c7b5f1efab34)
- [Vol.4: HOOK機能で「暴走RAG」を「答えないソクラテス」へ](https://zenn.dev/mizunadad/articles/c93e3c6ee8a436)
- [Vol.5: 疑似メタ認知アーキテクチャの全体設計](https://zenn.dev/mizunadad/articles/938465f3aea4ab)
